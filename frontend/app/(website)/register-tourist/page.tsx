...
interface ServiceDetails {
	...
	excursionType?: string[]; // now array for multi-select
	city: string;
...
export default function CombinedGuideBookingForm() {
	...
	const handleExcursionTypeChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
		const selected = Array.from(e.target.selectedOptions, (option) => option.value);
		setServiceDetails((prev) => ({ ...prev, excursionType: selected }));
	};
	...
	const calculateFees = useMemo((): FeeBreakdown => {
		...
		if (
			...
			serviceDetails.outstationDistance &&
			serviceDetails.outstationDistance > 100
		) {
			outstationExcursionAllowance = OUTSTATION_EXCURSION.standard;
			// Sum all selected excursions
			if (Array.isArray(serviceDetails.excursionType)) {
				serviceDetails.excursionType.forEach((type) => {
		...
		if (
			serviceDetails.isOutstation &&
			serviceDetails.outstationNights &&
			serviceDetails.outstationNights > 0
		) {
		...
		if (Array.isArray(serviceDetails.excursionType) && serviceDetails.excursionType.length > 0) {
			serviceDetails.excursionType.forEach((type) => {
				if (EXTRA_ALLOWANCE_EXCURSIONS.includes(type)) {
					extraAllowance += 1100;
				}
	...
	const handleBlur = (e: React.FocusEvent<HTMLInputElement | HTMLSelectElement>) => {
		...
		const value =
			name === 'languages' ? bookingData.languages : bookingData[name as keyof BookingFormData];
	...
	return (
		<div className='min-h-screen bg-gradient-to-br from-orange-50 via-amber-50 to-red-50 py-12 px-4 sm:px-6 lg:px-8 mt-20'>
			<div className='max-w-7xl mx-auto'>
				...
				<div className='text-center mb-8'>
					...
					<p className='text-sm text-orange-600 mt-2 font-medium'>
						IATO & TGFI Agreement | Valid Oct 2025 - Sep 2027
					</p>
				</div>
									...
									<div>
										...
										<div className='mt-2 p-2 bg-orange-50 rounded border border-orange-200'>
											<p className='text-xs text-gray-700'>
												...
												{bookingData.persons > 5 &&
													bookingData.persons <= 14 &&
													'₹3,250 (Half) / ₹4,000 (Full)'}
									...
									<div className='grid md:grid-cols-2 gap-4'>
										...
										<div className='bg-indigo-50 p-4 rounded-lg border border-indigo-200'>
											<label className='flex items-center gap-3 cursor-pointer'>
												<input
													type='checkbox'
													checked={serviceDetails.needsHotel}
													onChange={(e) => handleServiceChange('needsHotel', e.target.checked)}
													className='w-5 h-5 text-orange-600 rounded focus:ring-orange-500'
												/>
												<div>
													<p className='font-semibold text-gray-900'>Need Hotel</p>
													<p className='text-xs text-gray-600'>Assistance with hotel booking</p>
												</div>
											</label>
										</div>
										...
										<div className='bg-teal-50 p-4 rounded-lg border border-teal-200'>
											<label className='flex items-center gap-3 cursor-pointer'>
												<input
													type='checkbox'
													checked={serviceDetails.needsTaxi}
													onChange={(e) => handleServiceChange('needsTaxi', e.target.checked)}
													className='w-5 h-5 text-orange-600 rounded focus:ring-orange-500'
												/>
												<div>
													<p className='font-semibold text-gray-900'>Need Taxi Service</p>
													<p className='text-xs text-gray-600'>Local sightseeing & transfers</p>
												</div>
											</label>
										</div>
									</div>
									...
									<div className='bg-amber-50 p-4 rounded-lg border-2 border-amber-200'>
										<label className='flex items-center gap-3 cursor-pointer'>
											<input
												...
												onChange={(e) =>
													handleServiceChange('needsLanguageAllowance', e.target.checked)
												}
												className='w-5 h-5 text-orange-600 rounded focus:ring-orange-500'
											/>
											<div className='flex-1'>
												...
												<p className='text-xs text-gray-600 mt-1'>
													Additional allowance for non-English languages
												</p>
											</div>
										</label>
									</div>
									...
									<div className='bg-blue-50 p-4 rounded-lg border-2 border-blue-200'>
										<label className='flex items-center gap-3 cursor-pointer'>
											...
											<div className='flex-1'>
												...
												<p className='text-xs text-gray-600 mt-1'>
													For trips beyond 100km or overnight stays
												</p>
											</div>
										</label>
											...
											<div className='mt-4 space-y-3 ml-8'>
												<div>
													<label className='block text-xs font-medium text-gray-700 mb-1'>
														Distance (km){' '}
														<span className='text-orange-600'>• ₹2,200 if &gt;100km</span>
													</label>
													<input
														type='number'
														min='0'
												...
												<div>
													<label className='block text-xs font-medium text-gray-700 mb-1'>
														Overnight Stays{' '}
														<span className='text-orange-600'>• ₹2,250 - ₹5,500 per night</span>
													</label>
													<input
														type='number'
														min='0'
														...
														onChange={(e) =>
															handleServiceChange('outstationNights', parseInt(e.target.value) || 0)
														}
														className='w-full px-3 py-2 border border-gray-300 rounded-lg text-sm'
												...
												{(serviceDetails.outstationNights || 0) > 0 && (
													<label className='flex items-center gap-2 cursor-pointer bg-white p-2 rounded border border-blue-200'>
														<input
															...
															onChange={(e) =>
																handleServiceChange('accommodationProvided', e.target.checked)
															}
														...
														<div className='flex-1'>
															<div className='flex items-center justify-between'>
																...
																<span className='text-xs font-bold text-orange-600'>
																	{serviceDetails.accommodationProvided
																		? '₹2,250/night'
																		: '₹5,500/night'}
																</span>
															</div>
												...
												<div>
													...
													<select
														...
														onChange={handleExcursionTypeChange}
													...
													{Array.isArray(serviceDetails.excursionType) &&
														serviceDetails.excursionType.length > 0 && (
															<div className='mt-2 flex flex-wrap gap-2'>
																{serviceDetails.excursionType.map((type) => (
																	<span
																		key={type}
										...
										<div className='bg-purple-50 p-3 rounded-lg border border-purple-200'>
											<label className='flex items-center gap-2 cursor-pointer'>
												<input
													...
													onChange={(e) =>
														handleServiceChange('needsEarlyLateConveyance', e.target.checked)
													}
													className='w-4 h-4 text-orange-600 rounded'
												/>
												<div className='flex-1'>
													...
													<p className='text-xs text-gray-600 mt-0.5'>
														Before 7:30 AM or after 8:30 PM
													</p>
												</div>
											</label>
										</div>
										...
										<div className='bg-green-50 p-3 rounded-lg border border-green-200'>
											<label className='flex items-center gap-2 cursor-pointer'>
												<input
													...
													onChange={(e) =>
														handleServiceChange('needsExtraAllowance', e.target.checked)
													}
													className='w-4 h-4 text-orange-600 rounded'
												/>
												<div className='flex-1'>
													...
													<p className='text-xs text-gray-600 mt-0.5'>
														Report at hotel (applicable for select cities)
													</p>
												</div>
											</label>
										</div>
										...
										<div className='bg-pink-50 p-3 rounded-lg border border-pink-200'>
											<label className='flex items-center gap-2 cursor-pointer'>
												<input
													...
													onChange={(e) =>
														handleServiceChange('needsSpecialAllowance', e.target.checked)
													}
													className='w-4 h-4 text-orange-600 rounded'
												/>
												<div className='flex-1'>
											...
											{serviceDetails.needsSpecialAllowance && (
												<select
													...
													onChange={(e) =>
														handleServiceChange('specialAllowanceType', e.target.value)
													}
													className='w-full mt-2 px-3 py-2 border border-gray-300 rounded-lg text-sm'
												>
													<option value=''>Select event</option>
													<option value='kolkataGangaAarti'>Kolkata - Ganga Aarti (₹400)</option>
													<option value='kolkataVictoriaMemorial'>
														Kolkata - Victoria Memorial (₹400)
													</option>
													<option value='mumbaiElephanta'>Mumbai - Elephanta Tour (₹500)</option>
													<option value='varanasiMorningRiver'>
														Varanasi - Morning River (₹400)
													</option>
													<option value='varanasiEveningAarti'>
														Varanasi - Evening Aarti (₹500)
													</option>
												</select>
											)}
										</div>
								...
								<div className='space-y-3'>
									...
									<div className='flex justify-between items-center py-2 border-b border-orange-200'>
										...
										<span className='text-lg font-bold text-gray-900'>
											₹{calculateFees.baseFee.toLocaleString()}
										</span>
									</div>
									...
									{calculateFees.languageAllowance > 0 && (
										<div className='flex justify-between items-center py-2 border-b border-orange-200'>
											...
											<span className='text-lg font-bold text-gray-900'>
												₹{calculateFees.languageAllowance.toLocaleString()}
											</span>
										</div>
									)}
									...
									{calculateFees.outstationExcursionAllowance > 0 && (
										<div className='flex justify-between items-center py-2 border-b border-orange-200'>
											...
											<span className='text-lg font-bold text-gray-900'>
												₹{calculateFees.outstationExcursionAllowance.toLocaleString()}
											</span>
										</div>
									)}
									...
									{calculateFees.outstationAccommodationAllowance > 0 && (
										<div className='flex justify-between items-center py-2 border-b border-orange-200'>
											<span className='text-gray-700 font-medium'>
												Accommodation ({serviceDetails.outstationNights}{' '}
												{serviceDetails.outstationNights === 1 ? 'night' : 'nights'})
											</span>
											<span className='text-lg font-bold text-gray-900'>
												₹{calculateFees.outstationAccommodationAllowance.toLocaleString()}
											</span>
										</div>
									)}
									...
									{calculateFees.conveyanceAllowance > 0 && (
										<div className='flex justify-between items-center py-2 border-b border-orange-200'>
											...
											<span className='text-lg font-bold text-gray-900'>
												₹{calculateFees.conveyanceAllowance.toLocaleString()}
											</span>
										</div>
									)}
									...
									{calculateFees.extraAllowance > 0 && (
										<div className='flex justify-between items-center py-2 border-b border-orange-200'>
											...
											<span className='text-lg font-bold text-gray-900'>
												₹{calculateFees.extraAllowance.toLocaleString()}
											</span>
										</div>
									)}
									...
									{calculateFees.specialAllowance > 0 && (
										<div className='flex justify-between items-center py-2 border-b border-orange-200'>
											...
											<span className='text-lg font-bold text-gray-900'>
												₹{calculateFees.specialAllowance.toLocaleString()}
											</span>
										</div>
									)}
								</div>
				...
				<div className='mt-8 text-center'>
					<p className='text-sm text-gray-600'>
						Based on the agreement between Indian Association of Tour Operators (IATO) & Tourist
